...
- **Decision:** The registry provides a `register(bot: RegistryBot)` method that takes a minimal `RegistryBot` interface (not the grammY `Bot` class directly) and sets the bot's commands via `bot.api.setMyCommands()`. The registry does NOT set up message handlers or middleware — it only registers command metadata with Telegram's command menu. The `RegistryBot` interface follows the same DI pattern as `RegistryLogger` and `RegistryEventBus` — keeping the package framework-agnostic.
