...
- **Decision:** Utilize `neverthrow` (`Result<T, AppError>`) and strictly implement Rule XXXII (Redis Degradation) — falling back to Postgres on Redis failure, or in-memory if both fail.
