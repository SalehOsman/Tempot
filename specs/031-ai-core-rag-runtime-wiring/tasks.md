...
- [x] T001 Create isolated worktree branch `codex/031-rag-runtime-completion`
- [x] T002 Point `.specify/feature.json` at Spec #031
- [x] T003 Confirm Spec #029 and Spec #030 exports are accessible from public barrel
...
- [x] T004 [P] Add failing unit test: valid `RetrievalRequest` produces a `RetrievalOutcome`
      with selected block ids and stage timings - covers FR-001, FR-002, FR-005
- [x] T005 [P] Add failing unit test: request with no authorized content types returns
      `ok(outcome)` with empty `selectedBlockIds` - covers FR-001, FR-004
- [x] T006 [P] Add failing unit test: access-filter step records rejected block ids with
      reason `access-denied` - covers FR-004
- [x] T007 [P] Add failing unit test: embedding service failure returns
      `err(AppError)` with `ai-core.rag.search_failed` - covers FR-012
- [x] T008 [P] Add failing unit test: `buildAnswerState` with selected blocks returns
      `answered` with citations - covers FR-006, FR-007
- [x] T009 [P] Add failing unit test: `buildAnswerState` with no selected blocks returns
      `no-context` with non-blank `messageKey` - covers FR-008
- [x] T010 [P] Add failing unit test: `buildAnswerState` with `degraded: true` returns
      `degraded` with non-blank `messageKey` - covers FR-009
- [x] T011 [P] Add failing unit test: legacy `retrieve(RetrieveOptions)` still returns
      `ok(RAGContext)` without modification - covers FR-010
- [x] T012 [P] Add failing unit test: `retrieveWithPlan` logs `rag_search` audit entry
      when audit service dep is provided - covers FR-011
- [x] T013 [P] Add failing unit test: audit service failure does not propagate to caller
      - covers FR-011
- [x] T014 Run targeted Vitest test and verify RED for all new tests
...
- [x] T015 Create `packages/ai-core/src/rag/retrieval-plan.builder.ts`
...
- [x] T016 Create `packages/ai-core/src/rag/retrieval-plan.executor.ts`
...
- [x] T017 Extend `packages/ai-core/src/rag/rag-pipeline.service.ts`
...
- [x] T018 Add `ai-core.rag.no_context` and `ai-core.rag.degraded` i18n message key
      constants in `retrieval-plan.executor.ts` (as typed string literals, not free text)
- [x] T019 Run `pnpm --filter @tempot/ai-core test` and verify GREEN for all tests
...
- [x] T020 Export retrieval plan builder/executor functions from
...
- [x] T024 Update `CHANGELOG.md` `[Unreleased]` section with Spec #031 entry
- [x] T025 Update `.specify/feature.json` to close Spec #031 state
...
- [x] T026 Run `pnpm --filter @tempot/ai-core test` - verify SC-001, SC-002
- [x] T027 Run `pnpm lint` - verify SC-003
- [x] T028 Run `pnpm spec:validate` - verify SC-004
- [x] T029 Run `git diff --check` - verify SC-005
- [x] T030 Run `pnpm build` - verify `TelegramAssistantUI` and `IntentRouter`
